...
class FAQIndexer:
    ...
    def __init__(self):
        if not hasattr(self, "initialized"):
            ...
            self.min_score_threshold = (
                0.6  # Minimum cosine similarity score for a match
            )
    ...
    async def load_index(self) -> bool:
        ...
        try:
            if not os.path.exists(self.index_path):
                logger.warning(
                    "Sentence-transformer index not found. Creating new index..."
                )
            ...
            async with self._lock:
                ...
                self.questions = data.get(
                    "questions", [entry["question"] for entry in self.kb_data]
                )
            ...
            logger.info(
                f"Index loaded successfully. Version: {self.index_version}, "
        ...
        try:
            ...
            async with self._lock:
                ...
                self.embeddings = await asyncio.to_thread(
                    self.model.encode, documents, convert_to_tensor=True
                )
            ...
            logger.info(
                f"Index created successfully. Version: {self.index_version}, "
                f"Documents: {len(self.kb_data)}"
            )
            return True
    ...
    async def search(
        self, query: str, threshold: Optional[float] = None, top_k: Optional[int] = None
    ) -> List[Dict]:
        """
